...
import { Heart, Droplet, Star, TrendingUp, Loader } from "lucide-react";
...
const QuickStats = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        setLoading(true);
        const res = await getMyDonationStats();
    ...
    ? [
        {
          ...
          supportingText:
            stats.pendingOffers > 0
              ? `${stats.pendingOffers} offer${stats.pendingOffers > 1 ? "s" : ""} pending`
              : "Completed donations",
          trend: stats.totalDonations > 0 ? "up" : "neutral",
          color: "#C23B22",
        ...
        },
        {
          icon: Droplet,
          label: "Lives Helped",
          value: String(stats.livesHelped),
          supportingText: "Every donation can help save a life.",
          trend: "neutral",
          color: "#7A2F2F",
          ...
          supportingText:
            stats.reliabilityScore === null
              ...
              : stats.reliabilityScore >= 80
                ? "Excellent"
                : stats.reliabilityScore >= 50
                  ? "Good"
                  : "Needs improvement",
          trend: stats.reliabilityScore >= 80 ? "up" : "neutral",
          color: "#7A2F2F",
          bgGradient: "from-[#7A2F2F]/5 to-[#7A2F2F]/10",
        },
      ]
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {loading && (
          <div className="flex justify-center py-8">
            <Loader size={28} className="text-[#7A2F2F] animate-spin" />
          </div>
        )}
